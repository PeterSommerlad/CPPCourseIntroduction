...
std::map<Word, int> histogram(std::istream &in) {
	std::map<Word, int> result { };
	std::for_each(std::istream_iterator<Word> { in }, std::istream_iterator<Word> { }, [&result](auto const &word) {
		result[word]++;
	}
	);
	return result;
...
std::set<statistics::HistogramEntry> top(std::map<Word, int> const &occurrences, int n) {
	if (n < 0) {
