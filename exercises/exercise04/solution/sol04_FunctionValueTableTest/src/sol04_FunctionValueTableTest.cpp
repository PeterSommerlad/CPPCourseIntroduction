...
void testSimpleGenerateValueList(){

	ASSERT_EQUAL((std::vector<double>{1,2,3,4}), generateValueList(1.0,4.0,4));
}

void testGenerateValueListWithJustOneStep(){

	ASSERT_EQUAL((std::vector<double>{1}), generateValueList(1.0,1.0,0));
}
...
// this code would go into a main() function in the real question
	std::ostringstream out{};
	std::string const expected {R"abc(
...
void runAllTests(int argc, char const *argv[]){
	...
	s.push_back(CUTE(testSimpleGenerateValueList));
	s.push_back(CUTE(testGenerateValueListWithJustOneStep));
